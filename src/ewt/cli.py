...
def find_firmware(yaml_file: Path, project_name: str) -> Path | None:
    ...
    Only the factory image (bootloader + partition table + app, flashable at
    offset 0x0) is acceptable, since ESP Web Tools flashes at 0x0. The OTA
    app-only ``firmware.bin`` would leave the device without a bootloader.
    """
